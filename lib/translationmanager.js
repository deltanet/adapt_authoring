...
TranslationManager.prototype.getTranslatePlugin = function (type, cb) {
  ...
  var pluginManager = pluginmanager.getManager();
  pluginManager.getPlugin('translate', type, function (error, pluginInfo) {
    if (error) {
...
exports = module.exports = {
  ...
  sortContentObjects: function(list, parentId, sorted) {
      // remove parent
      let parentIndex = _.findIndex(list, { _id: parentId });
      if(parentIndex > -1) list.splice(_.findIndex(list, { _id: parentId }), 1);
      // recursively store children
      let thisChildren = _.where(list, { _parentId: parentId });
  ...
  getSourceLang: function(courseId, cb) {
    database.getDatabase(function(error, db) {
      if(error) return cb(error);

      db.retrieve('config', {_courseId: courseId}, { jsonOnly: true }, function(error, results) {
        if(error) return cb(error);
        if (!results || results.length === 0) return cb(error);
        return cb(null, results[0]._defaultLanguage);
      });
    });
  }
};
