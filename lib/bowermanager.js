...
BowerManager.prototype.installPlugin = function(pluginName, pluginVersion, callback) {
  ...
  var packageName = (pluginVersion == '*')
    ...
    rimraf(path.join(bowerOptions.directory, pluginName), function (err) {
      if (err) {
...
/**
 * Completely removes the specified Adapt plugin.
 * @param {string} pluginType - Type of plugin
 * @param {string} pluginName - The name of the Adapt plugin
 * @param {function} callback - Callback function
 */
BowerManager.prototype.destroyPlugin = function(pluginType, pluginName, callback) {
  origin.contentmanager.getContentPlugin(pluginType, function gotPlugin(error, plugin) {
    async.parallel([
      function clearBowercache(cb) {
        // TODO this dir should be dynamically generated
        fs.remove(path.join(
          app.configuration.getConfig('root').toString(),
          'plugins',
          'content',
          'bower',
          'bowercache',
      ...
      function removeAllVersions(cb) {
        fs.remove(plugin.bowerConfig.options.versionsFolder, cb);
      },
      function removeCode(cb) {
        fs.remove(path.join(
          origin.configuration.getConfig('root').toString(),
          Constants.Folders.Temp,
          origin.configuration.getConfig('masterTenantID').toString(),
          Constants.Folders.Framework,
          Constants.Folders.Source,
          plugin.bowerConfig.srcLocation,
          pluginName
        ), cb);
      },
      function removeDbData(cb) {
        origin.db.destroy(plugin.getPluginType(), { name: pluginName }, cb);
      }
    ], callback);
  });
}
