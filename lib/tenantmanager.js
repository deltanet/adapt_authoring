...
var CREATE_TENANT_WHITELIST = ['name', 'displayName'];
...
exports = module.exports = {
      ...
      mkdirp(path.join(configuration.tempDir, tenant._id.toString()), function (err) {
        ...
        fs.exists(path.join(configuration.serverRoot, FRAMEWORK_DIR), function(exists) {
          if (!exists) {
            ...
            frameworkhelper.cloneFramework(function(err, result) {
              if (err) {
  ...
  createTenant: function (tenant, callback) {
    ...
    database.getDatabase(function(err, db){
      ...
      // verify that the tenant name is not already in use
      db.retrieve('tenant', {name: tenant.name}, function (error, results) {
        if (error) {
          return callback(error);
        }

        //Make sure no tenants have the same tenant name
        if (results.length > 0) {
          return callback(new TenantCreateError('Tenant with that tenant name already exists'));
        }
        ...
        db.create('tenant', tenant, function (error, result) {
          ...
          db.update('tenant', result, databaseInfo, function(error, tenant) {
            if (error) {
  ...
  destroyTenant: function (tenantId, callback) {
    ...
    database.getDatabase(function(err, db){
      ...
      self.retrieveTenant({ _id: tenantId }, function (error, results) {
        ...
        if (results) {
          return self.updateTenant({ '_id': results._id }, { _isDeleted: true }, callback);
        }
