 ...
 * @return {string}
 */
OutputPlugin.prototype.publishSCORM = function (courseId, req, res, next) {
  logger.log('error', 'OutputPlugin#publishSCORM must be implemented by extending objects!');
  throw new Error('OutputPlugin#publishSCORM must be implemented by extending objects!');
};
...
OutputManager.prototype.setupRoutes = function () {
  ...
  rest.get('/output/:type/publishscorm/:courseid', function (req, res, next) {
    logger.log('info', 'About to publish');
    var type = req.params.type;
    var courseId = req.params.courseid;
    var mode = Constants.Modes.Publish;

    that.publishSCORM(type, courseId, mode, req, res, function (error, result) {
...
["preview", "publish", "publishSCORM"].forEach( function (el, index, array) {
