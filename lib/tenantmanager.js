...
var CREATE_TENANT_WHITELIST = ['name', 'displayName', 'translationEnabled', 'isMaster'];
var UPDATE_TENANT_WHITELIST = ['displayName', 'translationEnabled', 'isMaster'];
...
exports = module.exports = {
  ...
  createTenantFilesystem: function(tenant, callback) {
    ...
    fs.mkdirs(path.join(configuration.tempDir, tenant._id.toString()), function (err) {
      if (err) {
  ...
  createTenant: function (tenant, callback) {
    ...
    database.getDatabase(function(err, db){
      ...
      if (!tenant.translationEnabled) {
        tenant.translationEnabled = false;
      }
  ...
  updateTenant: function (search, update, callback) {
    ...
    Object.keys(update).forEach(function(key) {
      if (!UPDATE_TENANT_WHITELIST.includes(key)) {
        delete update[key];
      }
    });
