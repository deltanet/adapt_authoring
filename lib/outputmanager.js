...
var Constants = {
    CourseCollections : {
        ...
        'config': {
            ...
            omitProps: ['_theme', '_menu',  '_generateSourcemap',
...
OutputPlugin.prototype.sanitizeCourseJSON = function(json, next) {
  try {
    ...
    // Check the config contains the _enabledExtensions or _enabledComponents values
    if (configJson.hasOwnProperty('_enabledExtensions')|| configJson.hasOwnProperty('_enabledComponents')) {
      var includedPlugins = [];
      var menu = configJson.hasOwnProperty('_menu')
        ? configJson._menu
        : Constants.Defaults.MenuName;
      var theme = configJson.hasOwnProperty('_theme')
        ? configJson._theme
        : Constants.Defaults.ThemeName;

      // To being, ensure the theme and menu are compiled
      includedPlugins = [theme, menu];

      // Process the extensions
      if (configJson.hasOwnProperty('_enabledExtensions')) {
        for (var i in configJson._enabledExtensions) {
          includedPlugins.push(configJson._enabledExtensions[i].name);
        }
      }

      // Process the components
      if (configJson.hasOwnProperty('_enabledComponents')) {
        for (var i in configJson._enabledComponents) {
          includedPlugins.push(configJson._enabledComponents[i].name);
        }
      }

      // Append the 'build' property to config
      configJson.build = {
        includes: includedPlugins
      };
    }
            ...
            async.eachSeries(results, function(result, callback) {
              ...
              assetmanager.retrieveAsset({ _id: result._assetId }, function (error, assets) {
                ...
                if (assets && assets.length == 0) {
                  logger.log('warn', 'Asset ' + result._assetId + ' not found for course ' + courseId);
                  return callback();
                }
