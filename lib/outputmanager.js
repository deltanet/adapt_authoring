 ...
 * @return {string}
 */
OutputPlugin.prototype.publishToXAPI = function (courseId, req, res, next) {
  logger.log('error', 'OutputPlugin#publishToXAPI must be implemented by extending objects!');
  throw new Error('OutputPlugin#publishToXAPI must be implemented by extending objects!');
};
...
OutputManager.prototype.setupRoutes = function () {
  ...
  rest.get('/output/:type/publishxapi/:courseid', function (req, res, next) {
    logger.log('info', 'About to publish');
    var type = req.params.type;
    var courseId = req.params.courseid;
    var mode = Constants.Modes.Publish;

    that.publishToXAPI(type, courseId, mode, req, res, function (error, result) {
...
["preview", "publish", "publishToXAPI"].forEach( function (el, index, array) {
