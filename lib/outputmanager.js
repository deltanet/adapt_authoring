 ...
 * @return {string}
 */
OutputPlugin.prototype.publishOptions = function (courseId, req, res, next) {
  logger.log('error', 'OutputPlugin#publish must be implemented by extending objects!');
  throw new Error('OutputPlugin#publish must be implemented by extending objects!');
};
...
OutputManager.prototype.setupRoutes = function () {
  ...
  // Deltanet change -
  rest.post('/output/:type/publishOptions/:courseid', function (req, res, next) {
    logger.log('info', 'About to publish');
    var type = req.params.type;
    var courseId = req.params.courseid;
    var mode = Constants.Modes.Publish;

    that.publishOptions(type, courseId, mode, req, res, function (error, result) {
...
["preview", "publish", "publishOptions"].forEach( function (el, index, array) {
