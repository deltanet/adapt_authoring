...
exports = module.exports = {
  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function(req, res, next) {
      ...
      self.retrieveUser({ email: delta.email }, function(error, user) {
        permissions.hasPermission(currentUser._id, 'update', permissions.buildResourceString(user._tenantId, '/user/' + user._id), function (error, hasPermission) {
          if(!hasPermission) {
            res.statusCode = 401;
