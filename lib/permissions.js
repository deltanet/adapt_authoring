...
function PolicyStatementParser(action, resource, statements, callback) {
  ...
  callback(null, {
    ...
    parse: function (cb) {
      var cachedPermissions = usermanager.getSessionVariable('cachedPermissions') || {};
      ...
      usermanager.setSessionVariable('cachedPermissions', cachedPermissions);
      cb(null, allowed);
    }
  ...
  invalidateUserPermissions: function (next) {
    ...
    database.getDatabase(function (err, db) {
      ...
      db.update('user', { _id: currentUser._id }, { invalidatePermissionCache: false }, function (err) {
        ...
        usermanager.setSessionVariable('cachedPermissions', null);
      });
    }, configuration.getConfig('dbName'));
  },
