...
function getConfigJson(courseId, tenantId, callback) {
  ...
  origin().contentmanager.getContentPlugin('config', function (err, contentPlugin) {
    ...
    contentPlugin.retrieve({ _courseId: courseId }, { _tenantId: tenantId }, function(err, config) {
      if (err) {
        return callback(err);
      }
      if (config.length !== 1) {
...
OutputPlugin.prototype.generateIncludesForConfig = function(config, callback) {
  ...
  async.eachSeries(includedPlugins, function(plugin, next) {
    if (plugin.length < 2) {
      logger.log('error', 'Included plugins incorrectly specified.');
...
function generateIncludedPlugins(config) {
  ...
  const pluginTypeInfos = [
    ...
    for(var i in val) {
      ...
      var excludePlugin = (val[i]._excludeFromBuild);
