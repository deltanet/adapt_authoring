    ...
    async.each(Object.keys(Constants.CourseCollections), function(collectionType, callback) {
      ...
      if (collectionType == 'config') {
        ...
        origin().contentmanager.getContentPlugin('config', function (err, contentPlugin) {
          ...
          contentPlugin.retrieve(criteria, { _tenantId: tenantId }, function(err, config) {
            if (err) {
              return callback(err);
            }
            ...
            if (error) {
              ...
              db.exportResults(results, function (transformed) {
                ...
                transformed && transformed.forEach(function (item) {
                  flattenObject(item, '_extensions');
...
OutputPlugin.prototype.generateIncludesForCourse = function(courseId, next) {
    ...
    async.waterfall([
        ...
        function getIncludes(config, callback) {
            self.generateIncludesForConfig(config[0], function(error, includes) {
                if (error) {
                    callback(error);
                }
                if(!includes) {
                    callback(new Error("No plugins included for course " + courseId));
                } else {
...
OutputPlugin.prototype.generateIncludesForConfig = function(config, callback) {
  ...
  var includedPlugins = [
    [ Constants.Folders.Theme, theme ],
    [ Constants.Folders.Menu, menu ]
  ];

  // Process the extensions
  if (config.hasOwnProperty('_enabledExtensions')) {
    for (var i in config._enabledExtensions) {
      includedPlugins.push([Constants.Folders.Extensions, config._enabledExtensions[i].name]);
    }
  }
  ...
  if (config.hasOwnProperty('_enabledComponents')) {
    for (var i in config._enabledComponents) {
      includedPlugins.push([Constants.Folders.Components, config._enabledComponents[i].name]);
    }
  }
  ...
  async.eachSeries(includedPlugins, function(plugin, next) {
    ...
    self.addPluginWithDependencies(pluginNames, source_root, plugin[0], plugin[1], function() {
      return next();
    });
  }, function(error) {
    if (error) {
        ...
        fs.remove(destinationFolder, function(err) {
          ...
          fs.copy(themeFolder, destinationFolder, function (err) {
            ...
            if (themeSettings) {
              ...
              // There are no theme customisations, but the directory structure has been setup
...
OutputPlugin.prototype.writeCourseAssets = function(tenantId, courseId, destinationFolder, jsonObject, next) {
  async.waterfall([
    ...
    function emptyDest(cb) {
      rimraf(destinationFolder, function(error) {
        if(error) cb(error);
        fs.ensureDir(destinationFolder, cb);
      });
    },
    function getDb(dir, cb) {
      database.getDatabase(cb);
    },
    // retrieve a distinct list of assets.
    function getCourseAssets(db, cb) {
      db.retrieve('courseasset', { _courseId: courseId }, { operators: { distinct: '_assetId' } }, cb);
    },
    // retrieve every asset in the course, or return early if none
    function getAssets(results, cb) {
      if (results) {
        assetmanager.retrieveAsset({ _id: {$in: results} }, cb);
      } else {
        return next(null, jsonObject);
      }
    },
    // transform name of any assets to match the destinationFolder structure
    function processAssets(assets, cb) {
      // series to prevent write errors
      async.eachSeries(assets, function(asset, callback) {
        ...
        filestorage.getStorage(asset.repository, function (error, storage) {
          ...
          storage && storage.createReadStream(asset.path, function(ars) {
            var aws = fs.createWriteStream(outputFilename);
            ars.on('error', function(error) {
              cb('Error copying ' + asset.path + ' to ' + outputFilename + ": " + error);
            });
            ars.on('end', callback);
            ars.pipe(aws);
          });
...
OutputPlugin.prototype.applyMenu = function(tenantId, courseId, jsonObject, destinationFolder, next) {
  ...
  var menuName = jsonObject['config'].hasOwnProperty('_menu')
            ...
            fs.remove(destinationFolder, function (err) {
              ...
              fs.copy(menuFolder, destinationFolder, function (err) {
                if (err) {
                  ...
                  menuName = destinationFolder.replace(MENU_ROOT + path.sep,'');
