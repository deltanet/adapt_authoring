...
exports = module.exports = {
    ...
    return {
      ...
      moduleLoaded : function (modloaded) {
        ...
        if(modloaded === WAITFOR){
          ...
          rest.post('/asset/upload', instance.postAssetZip.bind(instance));
  ...
  retrieveAsset: function (search, options, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      // only return assets if user has correct permissions
      __this.hasPermission('read', user._id, user.tenant._id, '*', function(error, isAllowed) { // deltanet edit see #337
        if (error) {
          return next(error);
        }
        db.retrieve('asset', search, options, next);
      });
    }, options && options._tenantId); //Added for builder multi tenancy
  },
  ...
  trashAsset: function(id, next) {
    ...
    self.hasPermission('update', user._id, tenantId, { _id: id}, function (err, isAllowed) {
      if (!isAllowed) {
        return next(new AssetPermissionError());
      }
  ...
  /**
   * processes an uploaded asset zip file
   *
   * @param {object} res
   * @param {object} req
   * @param {callback} next
  ...
  postAssetZip: function (req, res, next) {
      ...
      form.parse(req, function (error, fields, files) {
        ...
        var isLimitedBrowser = (req.headers['accept'] && req.headers['accept'].indexOf('text/html') > -1) ? true : false;
        ...
        filestorage.getStorage(repository, function (error, storage) {
          ...
          var user = usermanager.getCurrentUser();
          var date = new Date();
          var globalFileDescription = fields.description;
          var globalTags = (fields.tags && fields.tags.length) ? fields.tags.split(',') : [];
          ...
          yauzl.open(file.path, { lazyEntries: true }, function(err, zipfile) {
            ...
            zipfile.on("entry", function(entry) {
              var dest = path.join(outputPath, entry.fileName);
              if (/\/$/.test(entry.fileName)) {
                ...
                zipfile.openReadStream(entry, function(error, readStream) {
                  ...
                  fs.ensureDir(path.dirname(dest), function(error) {
                    if (error) {
                      return next(error);
                    }
            ...
            zipfile.once('end', function() {
              // enumerate output directory and search for files
              fs.readdir(outputPath, function (err, files) {
                ...
                async.eachSeries(files, function(file, cb) {
                  ...
                  var fileStat = fs.statSync(path.join(outputPath, file));
                  if (fileStat.isFile()) {
                    // using hashes for file locations
                    // unzipFile must be object with name and path properties
                    var unzipFile = {
                      name: file,
                    ...
                    var rs = fs.createReadStream(unzipFile.path, {flags: 'r', autoClose: true});
                    ...
                    rs.on('close', function () {
                      ...
                      var fileExt = path.extname(unzipFile.name);
                      ...
                      storage.processFileUpload(unzipFile, filepath, fileOptions, function (error, storedFile) {
                        if (error) {
                          return next(error);
                        }
                        ...
                        var asset = _.extend({
                            title: file,
                            description: globalFileDescription,
  ...
  postAsset: function (req, res, next) {
    const _respondError = (message, code = 500) => {
    ...
    form.parse(req, function (error, fields, files) {
      if (error) {
        if (form.bytesExpected > form.maxFileSize) {
          return _respondError(app.polyglot.t('app.uploadsizeerror', {
            max: bytes.format(form.maxFileSize),
            size: bytes.format(form.bytesExpected)
          }), 400);
        }
        return _respondError(error.message);
      }
      ...
      filestorage.getStorage(repository, function (error, storage) {
        ...
        // feed into hasher
        rs.on('data', data => hash.update(data, 'utf8'));
        ...
        rs.on('close', function () {
          ...
          storage.processFileUpload(file, filepath, fileOptions, function (error, storedFile) {
            ...
            fs.remove(uploadedFilePath, function(error) {
              if (error) {
  ...
  assetThumb: function (req, res, next) {
    this.retrieveAsset({ _id: req.params.id }, function (error, assetRecs) {
      ...
      if (!assetRec.thumbnailPath || assetRec.thumbnailPath === 'false') {
        ...
        var img = Buffer.from(DEFAULT_THUMBNAIL_BASE64, 'base64');
        res.writeHead(200, {
