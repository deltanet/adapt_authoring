  ...
  database.getDatabase(function(err, db) {
    ...
    async.reduce(Object.keys(Constants.CourseCollections), {}, function(memo, collectionType, callback) {
      ...
      if (collectionType === 'config') {
        return getConfigJson(courseId, tenantId, callback);
      }
...
function getConfigJson(courseId, tenantId, callback) {
  // shuffle params
  if ('function' === typeof tenantId) {
    callback = tenantId;
    tenantId = configuration.getConfig('dbName');
  }
    ...
    contentPlugin.retrieve({ _courseId: courseId }, {}, function(err, config) {
      ...
      database.getDatabase(function(err, db) {
        ...
        db.retrieve('component', {_courseId: courseId}, {operators: {distinct: '_component'}}, function (err, components) {
          logger.log('info', JSON.stringify(components));
          // Retrieve the component types.
          database.getDatabase(function(err, db2) {
            if (err) {
              logger.log('error', err);
              return next(err);
            }
            db2.retrieve('componenttype', {}, function(err, componentTypes) {
              if (err) {
                return next(err);
              }
  ...
  try {
    ...
    async.waterfall([
      function(callback) {
        ...
        self.generateIncludesForConfig(mode, configJson, function(error, includes) {
          if (error) {
            return callback(error);
          }
...
OutputPlugin.prototype.generateIncludesForCourse = function(courseId, next) {
  async.waterfall([
    ...
    (json, callback) => {
      this.generateIncludesForConfig('EXPORT', json.config[0], (error, includes) => {
        if(error) {
          return callback(error);
        }
        if(!includes) {
...
OutputPlugin.prototype.generateIncludesForConfig = function(mode, config, callback) {
  var self = this;
  var pluginNames = [];
  ...
  var includedPlugins = generateIncludedPlugins(mode, config);

  if (!config.hasOwnProperty('_menu')) {
    config._menu = Constants.Defaults.MenuName;
  }

  if (!config.hasOwnProperty('_theme')) {
    config._theme = Constants.Defaults.ThemeName;
  }
...
function generateIncludedPlugins(mode, config) {
  ...
  const pluginTypeInfos = [
    ...
    for(var i in val) {
      // Deltanet change - exclude selected extensions from build
      var excludePlugin = (val[i]._excludeFromBuild && mode !== Constants.Modes.Export);
      if (typeof val[i].name !== 'undefined' && !excludePlugin) {
        includedPlugins.push([ pluginTypeInfo.folder, val[i].name ]);
      }
    }
  });
      ...
      database.getDatabase(function (err, db) {
        ...
        db.retrieve('courseasset', {_courseId: courseId, _contentType: {$ne: 'theme'}}, {operators: {distinct: '_assetId'}}, function (err, results) {
          ...
          if (results) {
            ...
            assetmanager.retrieveAsset({ _id: {$in: results} }, { _tenantId: tenantId }, function (error, assets) {  // deltanet edit for multi tenancy
              if (error) {
                logger.log('error', err);
                return next(error);
              }
