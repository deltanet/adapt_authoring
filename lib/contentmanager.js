...
ContentPlugin.prototype.retrieve = function (search, options, next) {
  ...
  var tenantId = options && options._tenantId || user.tenant && user.tenant._id; //Amended for builder multi tenancy DELTANET
  // must have a model name
  if (!this.getModelName()) {
    return next(new ContentTypeError('this.getModelName() must be set!'));
  }
  ...
  }, tenantId); //Amended for builder multi tenancy
};
