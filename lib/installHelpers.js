...
function checkFfmpegInstalled(callback) {
  if(!configuration.getConfig('useffmpeg')) {
    return callback();
  }
  execCommand('ffmpeg -version', {}, error => {
    callback(null, error && 'Local FFmpeg could not be found, please check it is installed' || null);
  });
}
