  ...
  rest.get('/content/schema', function (req, res, next) {
    ...
    database.getDatabase(function (err, db) {
      ...
      var plugins = [
        ...
        {collectionType: 'menutype', type: 'menu'}
      ];
      ...
      async.eachSeries(plugins, function (plugin, outerCallback) {
            ...
            function(callback) {
              async.eachSeries(results, function (schema, processCallback) {
                ...
                if (schema.globals) {
                  ...
                  switch (plugin.type) {
                    case 'component':
                      key = '_' + schema.component;
                      properties = coursePluginGlobals._components.properties;
                      break;

                    case 'extension':
                      key = '_' + schema.extension;
                      properties = coursePluginGlobals._extensions.properties;
                      break;
                    ...
                    case 'menu':
                      key = '_' + schema.menu;
                      properties = coursePluginGlobals._menu.properties;
                      break;
                  }
                ...
                switch (plugin.type) {
                  ...
                  case 'menu':
                    ...
                    async.each(Object.keys(schemaPluginLocations.properties), function(key, nestedCallback) {
                       var pluginLocations = schemaPluginLocations.properties[key];
                       if (pluginLocations.properties) {
                         // TODO only supports course, contentobject
                         // TODO don't like these hard-coded bits
                         contentModelExtras[key][schema.targetAttribute] = {
                          properties: pluginLocations.properties,
                          legend: schema.displayName,
                          name: schema.name,
                          type: "object"
                         };
                       }
                       ...
                       if (err) return processCallback(err);
                    });
        ...
        async.each(Object.keys(schemas), function(key, callback) {
          ...
          // extras
          if (contentModelExtras[key]) {
            var props = {};
            var keys = Object.keys(contentModelExtras[key]);
            for(var i = 0; i < keys.length; i++) {
              var PluginName = keys[i];
              props[PluginName] = contentModelExtras[key][PluginName];
            }
            value.properties.menuSettings = {
              type: "object",
              properties: props
            };
          }
