    ...
    database.getDatabase(function (err, db) {
      ...
      async.eachSeries(plugins, function (plugin, outerCallback) {
        ...
        db.retrieve(plugin.collectionType, null, function (err, results) {
          ...
          // This variable will be used to hold the most recent version of the collection type
          var newestResults = [];

          async.series([
            function(callback) {
              // Only use the most recent schemas, so remove anything older
              // Filter the results to only return the most recent versions
              async.eachSeries(results, function(currentRecord, filterCallback) {

                if (newestResults.length == 0) {
                  // Populate the frist
                  newestResults.push(currentRecord);
                } else {
                  var existing = _.find(newestResults, function(row) {
                    return row._doc.name == currentRecord._doc.name;
                  });

                  if (existing) {
                    if (semver.lt(existing._doc.version, currentRecord._doc.version)) {
                      // Remove the existing version and add the current one
                      newestResults = _.reject(newestResults, function(existing) { return existing._doc.name == currentRecord._doc.name });
            ...
            function(callback) {
              async.eachSeries(newestResults, function (schema, processCallback) {
