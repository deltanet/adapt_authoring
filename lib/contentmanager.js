 ...
 * @param {callback} next
 */
ContentPlugin.prototype.create = function (data, options, next) {
  ...
  database.getDatabase(function (error, db) {
    ...
    self.hasPermission('create', user._id, data._tenantId, data, function (err, isAllowed) {
      if (!isAllowed) {
        return next(new ContentPermissionError());
      }
 ...
 * @param {callback} next
 */
ContentPlugin.prototype.update = function (search, delta, options, next) {
 ...
 * @param {callback} next
 */
ContentPlugin.prototype.destroy = function (search, force, options, next) {
  var user = app.usermanager.getCurrentUser();
  var tenantId = null;
  // shuffle params
  ...
  if (options && options._tenantId) {
    tenantId = options._tenantId;
  } else {
    tenantId = user.tenant && user.tenant._id;
  }
...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/content/schema', function (req, res, next) {
    var schemas = helpers.cloneObject(that._contentSchemas);
    ...
    database.getDatabase(function (err, db) {
      ...
      var plugins = [
        { collectionType: 'componenttype', type: 'component', key: '_components' },
        { collectionType: 'extensiontype', type: 'extension', key: '_extensions' },
        { collectionType: 'themetype', type: 'theme', key: '_theme' },
        { collectionType: 'menutype', type: 'menu', key: '_menu' }
      ];
      ...
      async.eachSeries(plugins, function (plugin, outerCallback) {
          ...
          async.series([
            function(callback) {
              async.eachSeries(results, function (schema, processCallback) {
                ...
                if (schema.globals) {
                  // add plugin globals to the correct place in coursePluginGlobals
                  _.extend(coursePluginGlobals[plugin.key].properties, {
                    ['_' + schema[plugin.type]]: {
                      type: 'object',
                ...
                switch (plugin.type) {
                  ...
                  case 'theme':
                    ...
                    if(!schemaPluginLocations) break;

                    // TODO extension.pluginLocations should be required...
                    // logger.log('warn', plugin.type +' "' + schema.displayName + '" does not define required attribute "pluginLocations" ----------->');
                    ...
                    async.each(Object.keys(schemaPluginLocations.properties), function(key, nestedCallback) {
                      ...
                      var keys = Object.keys(pluginLocations.properties);
                      if(keys.length > 1) {
                        logger.log('error', 'Expected 1 property, found ' + Object.keys(pluginLocations.properties).length);
                        return nestedCallback();
                      }

                      // assume single top-level object
                      // needed to check against applied menu/theme in schemas.js
                      pluginLocations.properties[keys[0]].name = schema.name;
                      ...
                      switch(plugin.type) {
                        ...
                        case 'menu':
                          contentModelMenus[key] = _.extend(contentModelMenus[key], pluginLocations.properties);
                          break;
                      }
                    ...
                    // We don't want pluginLocations in any schemas,
                    // add plugins with any extras.
                    delete schema.properties.pluginLocations;
                    if(!_.isEmpty(schema.properties)) {
                      schemas[schema[plugin.type]] = schema;
                    }
        ...
        async.each(Object.keys(schemas), function(key, callback) {
          ...
          // extensions
          if (!_.isEmpty(contentModelExtensions[key])) {
            value.properties._extensions.properties = _.extend(value.properties._extensions.properties || {}, contentModelExtensions[key]);
          } else if (value.component) {
            if(!value.properties._extensions.properties) {
              value.properties._extensions.properties = contentModelExtensions.component;
            } else {
              value.properties._extensions.properties = _.extend(value.properties._extensions.properties, contentModelExtensions.component);
            }
          }

          // menus
          if (!_.isEmpty(contentModelMenus[key])) {
            if(!value.properties.menuSettings.properties) {
              value.properties.menuSettings.properties = contentModelMenus[key];
            } else {
              value.properties.menuSettings.properties = _.extend(value.properties.menuSettings.properties, contentModelMenus[key]);
            }
          }

          filteredSchemas[key] = _.omit(value.properties, blackList);
