...
var SELF_UPDATE_WHITELIST = ['firstName', 'lastName', '_isNewPassword', 'password'];
...
exports = module.exports = {
  ...
  createNewUser: function(user, callback) {
    ...
    auth.getAuthPlugin(configuration.getConfig('auth'), function (err, plugin) {
      ...
      plugin.internalRegisterUser(false, user, function(error, result) {
        ...
        // Deltanet change to default role
        app.rolemanager.assignRoleByName('Course Creator', result._id, function (err, userRecord) {
          if (err) {
            return callback(err);
          }
  ...
  init: function (app) {
    ...
    rest.get('/user/tenant', function(req, res, next) {
      var query = _.keys(req.body).length ? req.body : req.query || {};
      var options = query.search || {};
      options.populate = {
        'roles': 'name',
        '_tenantId': 'name'
      };
      ...
      self.hasPermission('update', user, '/api/user/resetpassword', function(err, isAllowed) {
        ...
        auth.hashPassword(delta.password, function(err, hash) {
          ...
          self.updateUser({email: delta.email}, delta, function(err) {
            ...
            app.mailer.send(delta.email, subject, body, { name: 'passwordReset/html.hbs' }, function(error) {
              res.statusCode = 200;
    ...
    rest.put('/user/me', function (req, res, next) {
      ...
      if (user) {
        if (user._id !== delta._id || user.email !== delta.email) {
          res.statusCode = 400;
    ...
    rest.post('/user', function (req, res, next) {
      ...
      self.hasPermission('create', currentUser, '/api/user', function(error, hasPermission) {
        if (!hasPermission) {
          res.statusCode = 401;
          return res.json({ success: false, message: 'Access denied' });
        }
    ...
    rest.delete('/user/:id', function (req, res, next) {
      ...
      self.retrieveUser({ _id: id }, function(error, user) {
        ...
        permissions.hasPermission(currentUser._id, 'delete', resourceStr, function (error, hasPermission) {
          ...
          self.deleteUser({ _id: id }, function(error) {
            // TODO error handling
            res.statusCode = 200;
            return res.json({ success: true });
          });
